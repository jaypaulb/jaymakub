...
# HAL Restore - Optional phase to restore SSH/GPG keys and configs from HAL backup server
...
# Main execution
echo
echo "=== HAL Restore Phase ==="
echo "Restoring SSH/GPG keys and configs from HAL (${HAL_HOST})"
echo
...
if check_hal_connection; then
    ...
    log_success "HAL restore phase complete"
else
    log_warn "Skipping HAL restore - server not reachable"
    log_info "You can manually run sysclone restore later"
fi
